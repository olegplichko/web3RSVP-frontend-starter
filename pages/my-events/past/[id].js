...
function PastEvent() {
  ...
  const confirmAttendee = async (attendee) => {
    try {
      ...
      if (rsvpContract) {
        const tnx = await rsvpContract.confirmAttendee(event.id, attendee);
        setLoading(true);
        console.log("Minting...", tnx.hash);

        await tnx.wait();
        console.log("Minted -- ", tnx.hash);
        setSuccess(ture);
        setLoading(false);
  ...
  const confirmAllAttendees = async () => {
    console.log("conrimAllAttendees");
    try {
      const rsvpContract = connectContract();
  ...
  function checkIfConfirmed(event, address) {
    for (let i = 0; i < event.confirmedAttendees.length; i++) {
      let confirmedAddress = event.confirmedAddress[i].attendee.id;
